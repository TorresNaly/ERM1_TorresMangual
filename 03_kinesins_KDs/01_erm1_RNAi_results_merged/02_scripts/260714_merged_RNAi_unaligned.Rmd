---
title: "erm-1 RNAi Treatments — Merged Analysis"
...
```
```{r linescans_mean_merged, fig.height = 10, fig.width = 14}
...
linescans_mean_merged <- ggplot(mean_signal,
       aes(x = aligned_row, y = mean_signal,
           group = channel)) +
  geom_ribbon(aes(ymin = ymeanmin, ymax = ymeanmax,
  ...
  labs(
    title = "Mean +- SD line scans, per treatment, overlaid by channel",
    x     = "Position",
    y     = "Mean normalized intensity",
    color = "channel",
    fill  = "channel"
...
linescans_mean_merged

```
```{r linescans_mean_merged_grid, fig.height = 14, fig.width = 12}
library(dplyr)

# Keep only the erm-1 channel (ch2)
df_ch2 <- mean_signal %>% filter(channel == "ch2")

# Separate treatments from the L4440 control
treatments  <- setdiff(unique(df_ch2$treatment), "L4440")
df_treat    <- df_ch2 %>% filter(treatment %in% treatments) %>%
  mutate(condition = "treatment")
df_control  <- df_ch2 %>% filter(treatment == "L4440")

# Replicate the L4440 control data into every treatment's facet panel
df_control_rep <- purrr::map_dfr(treatments, function(t) {
  df_control %>% mutate(treatment = t, condition = "control")
})

# Combine into one plotting dataframe
plot_data <- bind_rows(df_treat, df_control_rep)

condition_colors <- c(
  "treatment" = "#3E7CB1",  # blue
  "control"   = "gray30"    # gray
)
...
linescans_mean_merged_grid <- ggplot(plot_data,
       aes(x = aligned_row, y = mean_signal, group = condition)) +
  geom_ribbon(aes(ymin = ymeanmin, ymax = ymeanmax, fill = condition),
              alpha = 0.2, color = NA) +
  geom_line(aes(color = condition), linewidth = 0.8) +
  ...
  labs(
    title = "Mean +- SD line scans, erm-1 mRNA, RNAi vs L4440 control",
    x     = "Position",
    y     = "Mean normalized intensity",
    color = "condition",
    fill  = "condition"
...
ggsave(file.path(output_dir, paste0(today, "_linescan_individual.svg")), plot = p_linescan_individual,
       width = 14, height = 12)
ggsave(file.path(output_dir, paste0(today, "_linescan_mean_merged.svg")), plot = linescans_mean_merged,
       width = 14, height = 12)
ggsave(file.path(output_dir, paste0(today, "_linescan_mean_merged_grid.svg")), plot = linescans_mean_merged_grid,
       width = 14, height = 12)
